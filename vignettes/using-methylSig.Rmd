...
Prior to calling `diff_dss_test()`, it may help to look at the model matrix used for fitting in order to build the contrast.
...
The contrast passed to `diff_dss_test()` should be a vector with one value per column of the model matrix above, or a matrix whose rows correspond to those columns. A contrast of the wrong size gives an error that lists the columns of the model matrix, and `diff_dss_test()` gives a message with the hypothesis it tests, e.g. `Typenormal = 0`. Note that `Typenormal` is normal minus cancer, the reference level, so a positive `stat` is higher methylation in normal, while `meth_diff` below is case (cancer) minus control (normal). See the [DSS user guide](http://bioconductor.org/packages/release/bioc/vignettes/DSS/inst/doc/DSS.html#34_dmldmr_detection_from_general_experimental_design) for more information.
...
The `diff_dss_test()` function enables the recovery of group methylation rates via the optional `methylation_group_column` and `methylation_groups` parameters.
...
The numerical covariate test. Note, here the `methylation_groups` parameter is omitted because there are no groups. By giving the numerical covariate column, we will group samples by the top/bottom 25 percentile over the covariate, and compute mean methylation within those groups of samples. `meth_case` is the top group and `meth_control` the bottom group, so `meth_diff` is positive and `direction` is `Hyper` when methylation increases with the covariate. (Before methylSig 1.25.12 the groups were the reverse.) The `covariate_percentiles` parameter sets other percentiles, e.g. `c(10, 90)`.
