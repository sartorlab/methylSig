...
CpG methylation score files can be obtained using \verb@bismark@ with the flags \verb@--bedGraph$ and \verb@--cytosine_report@. An awk script can then be used on the cytosine report result to conform to the expected output.

\begin{verbatim}
$ awk '$4 + $5 > %s { print $1 "." $2 + 1 "\t" $1 "\t" $2 + 1 "\t" $3 "\t" $4 + $5 "\t" $4 "\t" $5 }' in.txt > out.txt
\end{verbatim}
